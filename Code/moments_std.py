...
class Moments_estimation(Env):
    ...
    def step(self, action):
        ...
        if self.std_step is None:
            mu2, sig2  = diffuse_state(dt = 1, mu = self.state[-2]-self.om0, 
                                   std = self.state[-1]*self.state[-2], noise = self.noise)
            self.state[-2] = mu2 + self.om0
            self.state[-1] = sig2/self.state[-2]
        else:
            self.state[-1] = (self.state[-1]*self.state[-2] + self.std_step)/self.state[-2]
