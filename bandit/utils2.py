...
def analyse_few_games4(results, string):
    ...
    cb = plt.colorbar(sig_plot, ax = axs[2], orientation="vertical")
    cb.set_label(r'Uncertainty $\sigma$')
    plt.tight_layout()
    plt.grid()
    axs[2].grid(True)
    axs[2].set_ylabel("Repetitions")
    ...
    #error
    dom = oms - mus
    om_plot = axs[3].pcolormesh(dom, cmap="bwr", vmin=-np.max(np.abs(dom)), vmax=np.max(np.abs(dom)))

    cb = plt.colorbar(om_plot, ax = axs[3], orientation="vertical")
    cb.set_label('Est. error')
    plt.tight_layout()
    axs[3].grid(True)
    plt.ylabel("Repetitions")
...
def analyse_time(results, string):
    ...
    a = ax.scatter(x,z,c=c, marker=".", alpha=0.5, cmap="Wistia")
    ax.legend()
    ax.set_xlabel(r"$\mu$")
